...
<%
	...
	if(!isValidate(out,id)) return;//아이디 미 입력시:서버단에서도 유효성 체크
					...
					<div class="content">
						...
						<table
							...
							<tr style="background-color: white">
								<td>아이디</td>
								<td><input type="text" name="id" size="20" value="<%=id%>"/> <span
							...
							<tr style="background-color: white">
								...
								<td><input type="radio" name="gender" value="man" <%if("MAN".equals(gender.toUpperCase())){ %> checked <%}%>/>남 
								    <input type="radio" name="gender" value="woman" <%if("WOMAN".equals(gender.toUpperCase())){ %> checked <%}%>/>녀 
								    <input type="radio" name="gender" value="trans" <%if("TRANS".equals(gender.toUpperCase())){ %> checked <%}%>/>트랜스젠더</td>
							</tr>
							<tr style="background-color: white">
								<td>관심사항</td>
							...
							<tr style="background-color: white">
								...
								<td><select name="grade">										
										<option value="ele" <% if("ELE".equals(grade.toUpperCase())){ %>selected<%} %>>초등학교</option>
										<option value="mid" <% if("MID".equals(grade.toUpperCase())){ %>selected<%} %>>중학교</option>
										<option value="hig" <% if("HIG".equals(grade.toUpperCase())){ %>selected<%} %>>고등학교</option>
										<option value="uni" <% if("UNI".equals(grade.toUpperCase())){ %>selected<%} %>>대학교</option>
								</select></td>
							</tr>
							<tr style="background-color: white">
								<td>첨부파일</td>
