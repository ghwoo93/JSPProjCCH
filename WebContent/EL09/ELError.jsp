...
<body>
	<fieldset>
		...
		<h2>배열이나 리스트 계열 컬렉션의 인덱스를 벗어난 경우(에러 NO,출력 NO)</h2>
		<ul>
		...
		<%
			//리퀘스트 영역에 저장]
			request.setAttribute("myrequest","리쿼스트 영역");
			//맵 컬렉션 객체 생성]
			Map map = new HashMap();
			map.put("mymap","맵 컬렉션");
		...
		<ul>
			...
			<li>\${requestScope.yourrequest }[에러NO,출력 NO] : ${requestScope.yourrequest }</li>
			<li>\${map.mymap }[정상] : ${map.mymap }</li>
			<li>\${map.yourmap }[에러NO,출력 NO] : ${map.yourmap }</li>
