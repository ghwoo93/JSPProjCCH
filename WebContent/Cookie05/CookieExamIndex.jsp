...
<%
	...
	if(cookies !=null){
		for(Cookie cookie:cookies){
			...
			//아이디 저장용 쿠키 체크]
			if(name.equals("USER_ID")) username=value;
		}
	}
...
<body>
	<fieldset>
		...
		<form action="CookieExamCartProcess.jsp">
			<input type="checkbox" name="cart" value="product1" <% if(cart.contains("product1")){  %> disabled checked<%}//if %>/>상품1 
			<input type="checkbox" name="cart" value="product2" <% if(cart.contains("product2")){  %> disabled checked<%}//if %>/>상품2 
			<input type="checkbox" name="cart" value="product3" <% if(cart.contains("product3")){  %> disabled checked<%}//if %>/>상품3 
			<input type="checkbox" name="cart" value="product4" <% if(cart.contains("product4")){  %> disabled checked<%}//if %>/>상품4 
			<input type="checkbox" name="cart" value="product14" <% if(cart.contains("product14")){  %> disabled checked<%}//if %>/>상품14 
			<input type="checkbox" name="cart" value="product190" <% if(cart.contains("product190")){  %> disabled checked<%}//if %>/>상품190 
	...
	<fieldset>
		...
		​ ​ <span style="color: red; font-weight: bold"><%=request.getAttribute("ERROR")==null?"":request.getAttribute("ERROR") %></span>
			...
			<table
				style="width: 400px; border-spacing: 1px; background-color: green">
